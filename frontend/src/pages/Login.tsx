/** Sign-in / first-run setup. The backend reports whether any user exists;
 * first run shows the "create your account" flow. */
    ...
    <div className="grid min-h-full place-items-center p-4">
      <div className="fade-up w-full max-w-sm">
        <div className="mb-6 text-center">
          ...
          <h1 className="mt-3 text-2xl font-bold text-ink-1">RigBooks</h1>
          <p className="mt-1 text-sm text-ink-3">
            {needsSetup
              ? "Welcome! Create your local account to get started."
              : "Sign in to your books. Everything stays on this machine."}
          </p>
        </div>
        <form onSubmit={submit}
          ...
          {needsSetup && (
            <Field label="Your name">
              <Input value={name} onChange={(e) => setName(e.target.value)}
                placeholder="e.g. Lilibeth" required />
            </Field>
          )}
          <Field label="Email">
          ...
          <Field label={needsSetup ? "Password (8+ characters)" : "Password"}>
            <Input type="password" value={password}
              onChange={(e) => setPassword(e.target.value)} required minLength={needsSetup ? 8 : 1} />
          </Field>
        ...
        <p className="mt-4 text-center text-xs text-ink-3">
          Local-first · your data never leaves this computer
        </p>
      </div>
    </div>
  );
