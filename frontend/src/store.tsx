/** Global app state: auth session, theme, reporting period. */
import {
...
function taxYear(year: number): Period {
  return { start: `${year}-01-01`, end: `${year}-12-31`, label: `Tax year ${year}` };
}
...
export function StoreProvider({ children }: { children: ReactNode }) {
  ...
  const [dark, setDark] = useState(
    () => localStorage.getItem("rigbooks_theme") === "dark" ||
      (localStorage.getItem("rigbooks_theme") === null &&
        window.matchMedia("(prefers-color-scheme: dark)").matches),
  );
  const [period, setPeriod] = useState<Period>(() => taxYear(new Date().getFullYear()));

  useEffect(() => {
    document.documentElement.classList.toggle("dark", dark);
    localStorage.setItem("rigbooks_theme", dark ? "dark" : "light");
  }, [dark]);
  ...
  const value = useMemo(
    () => ({
      ...
      year: new Date(period.start + "T00:00:00").getFullYear(),
    }),
